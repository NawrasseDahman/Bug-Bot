  ...
  execute: function(bot, channelID, userTag, userID, command, msg, trello, db) {
    ...
    db.get("SELECT " + whichOS + " FROM users WHERE userid = ?", [userID], function(error, dbRowReply){
      ...
      if(!!dbRowReply){
        ...
        bot.createMessage(config.channel.modLogChannel, ":floppy_disk: **" + userTag + "** changed **" + utils.toTitleCase(whichOS) + "** -> `" + systemInfo + "`"); //user changed their --- info
      }else if(!dbRowReply){
        //Create new database entry
        db.run("INSERT INTO users (userid, " + whichOS + ") VALUES(?, ?)", [userID, systemInfo]);
        utils.botReply(bot, userID, channelID, "your new " + utils.toTitleCase(whichOS) + " settings have been saved", command, msg.id);
        bot.createMessage(config.channel.modLogChannel, ":floppy_disk: **" + userTag + "** added **" + utils.toTitleCase(whichOS) + "** -> `" + systemInfo + "`"); //user added their --- info
      }
    });
  },
  roles: [
